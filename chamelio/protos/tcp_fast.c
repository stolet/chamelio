#include <string.h>
#include <linux/types.h>

#include <rte_ip4.h>

#include "queue_fns.h"
#include "scheduler_fns.h"
#include "clock.h"
#include "utils_sync.h"
#include "log.h"

#include "cham_fast.h"
#include "tcp_hdr.h"
#include "ip_hdr.h"
#include "tcp.h"
#include "tcp_payload_trace.h"
#include "tcp_queue_types.h"
#include "utils.h"

#define PORT_MAP 0
#define SOCK_MAP 1
#define SID_BUCK_MAP 2
#define CFG_MAP 3
#define TCP_MAX_PAYLOAD (FAST_L3_PKT_ROOM - sizeof(struct tcp_pkt_inner))
#define FAST_REMIT_THRESH 5

/* Miscelaneous helpers */
static inline __u8 is_qe_valid(struct queue_entry *qe, void *shm_end);  
static inline __u8 is_qe_bump_valid(struct queue_entry *qe,
    void *shm_end);
static inline __u8 is_qe_ctl_valid(struct queue_entry *qe,
    void *shm_end);
static inline __u8 is_qe_pkt_valid(struct queue_entry *qe,
    void *shm_end);
static inline __u8 is_pktlen_valid(void *pkt, void *pkt_end);
static inline __u8 is_paylen_valid(void *pkt, 
    void *pkt_end, __u16 hdrs_len, __u16 paylen);
static inline void remit(struct cham_scheduler *sched,
    struct tcp_sock *sock);
  
/* RX helpers */
static inline __u32 rx_get_hdrlen(struct tcp_pkt_inner *tcp_pkt);
static inline __u32 rx_get_paylen(struct tcp_pkt_inner *tcp_pkt);
static inline __u8 rx_is_ip(struct tcp_pkt_inner *tcp_pkt);
static inline __u8 rx_is_ce(struct tcp_pkt_inner *tcp_pkt);
static inline __u8 rx_is_flags_ctl(struct tcp_pkt_inner *tcp_pkt);
static inline __u8 rx_is_ack(struct tcp_pkt_inner *tcp_pkt);
static inline __u8 rx_is_ack_only(struct tcp_pkt_inner *tcp_pkt);
static inline __u8 rx_is_payload_dup(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock);
static inline __u8 rx_is_out_of_order(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock);
static inline __u8 rx_is_ack_valid(__u32 seqno,
      __u32 pending, __u32 avail, __u32 ackno);
static inline int rx_punt_ctl(struct cham_ebpf_ctx *ctx,
    struct tcp_pkt_inner *tcp_pkt);
static inline __u32 rx_get_ack_bump(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock);
static inline __u8 rx_read_ts(struct tcp_pkt_inner *tcp_pkt,
    void *pkt_end, __u32 *ts_val, __u32 *ts_ecr);
static inline __u32 tcp_now_us(void);
static inline void sock_ts_rx(struct tcp_sock *sock,
    struct tcp_pkt_inner *tcp_pkt, void *pkt_end);
static inline void rx_sched_tx(struct cham_ebpf_ctx *ctx,
    struct tcp_sock *sock, __u32 avail_before, __u32 avail_after);
static inline __u32 rx_get_rx_bump(struct tcp_sock *sock, __u32 paylen);
static inline void rx_sock_bump_ack(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock, __u32 ack_bump);
static inline void rx_sock_bump_rx(struct tcp_sock *sock, 
    __u32 rx_bump);
static inline __u32 rx_get_payload_overlap(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock);
static inline void rx_copy_payload(void *shm_base,
    struct tcp_sock *sock, void *payload, __u32 paylen);
static inline int rx_enqueue_rx_bump(struct equeue *q,
    struct tcp_sock *sock, __u32 rx_bump, __u16 rx_port, __u32 rx_ip);
static inline int rx_enqueue_tx_bump(struct equeue *q,
    struct tcp_sock *sock, __u32 tx_bump);

/* TX helpers */
static inline __u32 tx_get_hdrlen();
static inline int tx_fill_ack(struct cham_ebpf_ctx *ctx,
    struct tcp_sock *sock);
static inline __u32 tx_get_max_paylen(struct cham_ebpf_ctx *ctx,
    __u32 hdrlen);
static inline void tx_write_tsopt(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock, __u32 now_us);
static inline __u32 tx_get_tx_bump(struct tcp_sock *sock,
    __u32 max_paylen);
static inline void tx_fill_hdr(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock, __u32 hdrlen, __u32 paylen);
static inline void tx_copy_payload(void *shm_base, struct tcp_sock *sock,
    void *payload, __u32 tx_bump);
static inline void tx_sock_tx_bump(struct tcp_sock *sock,
    __u32 tx_bump);
static inline __u64 tx_sched_tsc(struct tcp_sock *sock);
static inline void tx_sock_sched_next(struct tcp_sock *sock,
    __u32 pktlen, __u64 now_tsc);

/* DEQ helpers */
static inline int deq_handle_bump_tx(struct cham_ebpf_ctx *ctx);
static inline int deq_handle_bump_rx(struct cham_ebpf_ctx *ctx);
static inline int deq_handle_ctl_tx(struct cham_ebpf_ctx *ctx);
static inline int deq_handle_retransmit(struct cham_ebpf_ctx *ctx);
static inline __u8 deq_is_retransmit_stale(struct tcp_sock *sock,
    struct tcp_queue_ctl_remit *cmd);
static inline __u32 deq_get_tx_ip(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_tx *bump);
static inline __u32 deq_get_tx_port(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_tx *bump);
static inline __u32 deq_sock_bump_tx(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_tx *bump);
static inline __u32 deq_sock_bump_rx(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_rx *bump);
    
/* Socket helpers */
static inline struct tcp_sock * sock_find(struct cham_ebpf_ctx *ctx,
    struct tcp_pkt_inner *tcp_pkt);
static inline struct tcp_sock *sock_map_lookup(struct cham_map *map,
    __u32 sid);
static inline __u32 sock_hash(__u32 lip, __u16 lport,
    __u32 rip, __u16 rport);
static inline __u32 sock_sched_avail(struct tcp_sock *sock);

static inline __u64 rate_delay_tsc(__u32 bytes, __u32 rate_kbps)
{
  __u64 cycles_per_us;
  __u64 nr;

  if (bytes == 0 || rate_kbps == 0)
    return 0;

  cycles_per_us = clock_us_to_tsc(1);
  if (cycles_per_us == 0)
    return 0;

  nr = (__u64) bytes * 8 * 1000 * cycles_per_us;
  return (nr + rate_kbps - 1) / rate_kbps;
}

int tcp_event_rx(struct cham_ebpf_ctx *ctx)
{
  int ret;
  __u8 is_control, is_ack, is_ack_only, is_ack_dup, is_ip, is_out_of_order;
  __u8 is_payload_dup;
  __u8 ack_valid, should_fast_remit;
  __u8 pktlen_valid, paylen_valid;
  __u16 paylen, hdrlen;
  __u32 ack_bump, avail_before, avail_after, overlap, rx_bump, rx_tail;
  __u64 copy_start_tsc, copied_tsc;
  __u8 *rx_base;
  struct tcp_sock *sock;
  struct tcp_pkt_inner *tcp_pkt;
  void *full_payload, *payload, *rx_payload;
  
  /* Do pkt len check to make ebpf verifier happy */
  pktlen_valid = is_pktlen_valid(ctx->pkt, ctx->pkt_end);
  if (!pktlen_valid)
    return -1;
  tcp_pkt = (struct tcp_pkt_inner *) ctx->pkt;

  is_ip = rx_is_ip(tcp_pkt);
  if (!is_ip)
    return -1;

  /* Send control packets to slow path */
  is_control = rx_is_flags_ctl(tcp_pkt);
  if (is_control)
  {
    ret = rx_punt_ctl(ctx, tcp_pkt);
    return ret;
  }
  
  /* Get socket and drop packet if we can't find a socket */
  sock = sock_find(ctx, tcp_pkt);
  if (sock == NULL)
    return -1;

  if (sock->state != TCP_SOCK_STATE_ESTABLISHED)
    return rx_punt_ctl(ctx, tcp_pkt);
  
  /* Process control window and update remote available */
  util_spin_lock(&sock->lock);
  if (rx_is_ce(tcp_pkt))
    sock->flags |= TCP_SOCK_FLAG_SEND_ECE;

  hdrlen = rx_get_hdrlen(tcp_pkt);
  paylen_valid = is_paylen_valid(ctx->pkt, ctx->pkt_end, hdrlen, 0);
  if (!paylen_valid)
  {
    ret = tx_fill_ack(ctx, sock);
    util_spin_unlock(&sock->lock);
    return ret;
  }
  paylen = rx_get_paylen(tcp_pkt);
  paylen_valid = is_paylen_valid(ctx->pkt, ctx->pkt_end, hdrlen, paylen);
  full_payload = ctx->pkt + hdrlen;

  /* Get current state before processing ack */
  avail_before = sock_sched_avail(sock);
  sock->tx_remote_avail = f_beui16(tcp_pkt->tcp.wnd);
  is_ack = rx_is_ack(tcp_pkt);
  is_ack_dup = sock->tx_seq == f_beui32(tcp_pkt->tcp.ackno);
  is_ack_only = rx_is_ack_only(tcp_pkt);
  is_payload_dup = rx_is_payload_dup(tcp_pkt, sock);
  is_out_of_order = rx_is_out_of_order(tcp_pkt, sock);
  overlap = rx_get_payload_overlap(tcp_pkt, sock);
  if (is_ack)
    sock->cc_acks++;
  sock_ts_rx(sock, tcp_pkt, ctx->pkt_end);
  
  /* Process ACK bump */
  ack_valid = rx_is_ack_valid(sock->tx_seq, sock->tx_pending, sock->tx_avail,
      f_beui32(tcp_pkt->tcp.ackno));
  ack_bump = rx_get_ack_bump(tcp_pkt, sock);
  if (paylen_valid && paylen > 0)
  {
    if (is_ack && !ack_valid)
      tcp_payload_trace_add_to_msgs(full_payload, paylen,
          TCP_PAYLOAD_TRACE_TCP_INVALID_ACK, sock->id, 0, ctx->core);
    else if (is_ack && ack_bump > 0)
      tcp_payload_trace_add_to_msgs(full_payload, paylen,
          TCP_PAYLOAD_TRACE_TCP_ACK_PROCESSED, sock->id, ack_bump, ctx->core);
  }
  if (is_ack)
    rx_sock_bump_ack(tcp_pkt, sock, ack_bump);
  avail_after = sock_sched_avail(sock);
  
  /* Enqueue app bump if this was not a duplicate ACK */
  if (!is_ack_dup)
  {
    ret = rx_enqueue_tx_bump(&ctx->equeues[sock->app_bump_qid].eq, 
        sock, ack_bump);
    if (ret != 0)
    {
      util_spin_unlock(&sock->lock);
      return -1;
    }
  }
  
  /* If ack is duplicate check if we should retransmit */
  should_fast_remit = 0;
  if (is_ack && is_ack_only && ack_bump == 0 && 
      sock->tx_pending != 0 && !sock->recovery_active)
  {
    sock->rx_dupack_cnt++;
    should_fast_remit = sock->rx_dupack_cnt >= FAST_REMIT_THRESH;
    if (should_fast_remit)
    {
      sock->cc_drops++;
      remit(&ctx->sched, sock);
      sock->rx_dupack_cnt = 0;
    }
  }

  rx_sched_tx(ctx, sock, avail_before, avail_after);
  
  /* ACK-only packets should not trigger a response. */
  if (is_ack_only)
  {
    util_spin_unlock(&sock->lock);
    return 0;
  }

  /* Fully duplicate payload still needs an ACK */
  if (is_payload_dup && !should_fast_remit)
  {
    if (paylen_valid && paylen > 0)
      tcp_payload_trace_add_to_msgs(full_payload, paylen,
          TCP_PAYLOAD_TRACE_TCP_DUP_PAYLOAD, sock->id, paylen, ctx->core);
    ret = tx_fill_ack(ctx, sock);
    util_spin_unlock(&sock->lock);
    return ret;
  }
  
  /* Out-of-order payload still triggers a pure ACK. */
  if (is_out_of_order)
  {
    if (paylen_valid && paylen > 0)
      tcp_payload_trace_add_to_msgs(full_payload, paylen,
          TCP_PAYLOAD_TRACE_TCP_OUT_OF_ORDER, sock->id, paylen, ctx->core);
    ret = tx_fill_ack(ctx, sock);
    util_spin_unlock(&sock->lock);
    return ret;
  }
  
  /* Check if payload len valid so ebpf verifier is happy */
  if (!paylen_valid)
  {
    ret = tx_fill_ack(ctx, sock);
    util_spin_unlock(&sock->lock);
    return ret;
  }
  
  /* Trim duplicate prefix so only unseen bytes enter RX ring */
  payload = ctx->pkt + hdrlen + overlap;
  paylen -= overlap;
  
  /* Calculate bump for rx ring */
  rx_bump = rx_get_rx_bump(sock, paylen);
  if (rx_bump < paylen)
    tcp_payload_trace_add_to_msgs(payload, paylen,
        TCP_PAYLOAD_TRACE_TCP_QUEUE_FULL, sock->id, rx_bump, ctx->core);
  
  /* Copy payload to rx buf and enqueue bump */
  if (rx_bump > 0)
  {
    rx_payload = NULL;
    rx_base = (__u8 *) ctx->shm_base + sock->rx_off;
    rx_tail = sock->rx_head + sock->rx_avail;
    if (rx_tail >= sock->rx_len)
      rx_tail -= sock->rx_len;
    if (rx_tail + rx_bump <= sock->rx_len)
      rx_payload = rx_base + rx_tail;

    tcp_payload_trace_add_to_msgs(payload, rx_bump,
        TCP_PAYLOAD_TRACE_TCP_RX_DELIVERED, sock->id, rx_bump, ctx->core);
    copy_start_tsc = tcp_payload_trace_rdtsc();
    tcp_payload_trace_add_tsc_to_msgs(payload, rx_bump,
        TCP_PAYLOAD_TRACE_TCP_RX_RING_COPY_START, sock->id, rx_bump, ctx->core,
        copy_start_tsc);
    rx_copy_payload(ctx->shm_base, sock, payload, rx_bump);
    copied_tsc = tcp_payload_trace_rdtsc();
    if (rx_payload != NULL)
      tcp_payload_trace_add_tsc_to_msgs(rx_payload, rx_bump,
          TCP_PAYLOAD_TRACE_TCP_RX_RING_COPIED, sock->id, rx_bump, ctx->core,
          copied_tsc);
    else
      tcp_payload_trace_add_tsc_to_msgs(payload, rx_bump,
          TCP_PAYLOAD_TRACE_TCP_RX_RING_COPIED, sock->id, rx_bump, ctx->core,
          copied_tsc);
    rx_sock_bump_rx(sock, rx_bump);
    ret = rx_enqueue_rx_bump(&ctx->equeues[sock->app_bump_qid].eq, sock, rx_bump,
        f_beui16(tcp_pkt->tcp.src), f_beui32(tcp_pkt->ip.src));
    if (ret != 0)
    {
      util_spin_unlock(&sock->lock);
      return -1;
    }
    tcp_payload_trace_add_to_msgs(rx_payload != NULL ? rx_payload : payload,
        rx_bump, TCP_PAYLOAD_TRACE_TCP_RX_ENQUEUED, sock->id, rx_bump,
        ctx->core);
  }
  
  ret = tx_fill_ack(ctx, sock);
  util_spin_unlock(&sock->lock);
  
  return ret;
}

int tcp_event_sched(struct cham_ebpf_ctx *ctx)
{
  int ret;
  __u8 pktlen_valid, paylen_valid;
  __u64 copy_start_tsc, copied_tsc, now_tsc;
  __u32 hdrlen, max_paylen, tx_bump, avail, sid;
  void *payload;
  struct cham_map *map;
  struct tcp_sock *sock;
  struct tcp_pkt_inner *tcp_pkt;
  struct cham_sched_entry *sched_entry;
  
  /* Check if scheduler is empty */
  sched_entry = sched_head(&ctx->sched);
  if (sched_entry == NULL)
    return 0;

  now_tsc = clock_rdtsc();
  if (sched_entry->priority > now_tsc)
    return 0;
  
  /* Pop top scheduler entry */
  sid = sched_entry->id;
  ret = sched_pop(&ctx->sched);
  if (ret != 0)
    return -1;
  
  /* Get socket */
  map = &ctx->maps[SOCK_MAP];
  sock = sock_map_lookup(map, sid);
  if (sock == NULL)
    return -1;
  
  /* Do pkt len check to make ebpf verifier happy */
  pktlen_valid = is_pktlen_valid(ctx->pkt, ctx->pkt_end);
  if (!pktlen_valid)
    return -1;
  tcp_pkt = (struct tcp_pkt_inner *) ctx->pkt;
  
  /* Check if payload len valid so ebpf verifier is happy */
  util_spin_lock(&sock->lock);
  hdrlen = tx_get_hdrlen();
  max_paylen = tx_get_max_paylen(ctx, hdrlen);
  tx_bump = tx_get_tx_bump(sock, max_paylen);
  paylen_valid = is_paylen_valid(ctx->pkt, ctx->pkt_end, hdrlen, tx_bump);
  if (!paylen_valid)
  {
    util_spin_unlock(&sock->lock);
    return -1;
  }

  /* Fill packet header for transmission */
  tx_fill_hdr(tcp_pkt, sock, hdrlen, tx_bump);
  
  /* Fill payload if this is not a pure ACK */
  if (tx_bump > 0)
  {
    /* Copy payload from TX buffer to packet and update socket */
    payload = ctx->pkt + hdrlen;
    copy_start_tsc = tcp_payload_trace_rdtsc();
    tx_copy_payload(ctx->shm_base, sock, payload, tx_bump);
    copied_tsc = tcp_payload_trace_rdtsc();
    tcp_payload_trace_add_tsc_to_msgs(payload, tx_bump,
        TCP_PAYLOAD_TRACE_TCP_SCHED_COPY_START, sock->id, tx_bump, ctx->core,
        copy_start_tsc);
    tcp_payload_trace_add_tsc_to_msgs(payload, tx_bump,
        TCP_PAYLOAD_TRACE_TCP_SCHED_COPY_END, sock->id, tx_bump, ctx->core,
        copied_tsc);
    tcp_payload_trace_add_to_msgs(payload, tx_bump,
        TCP_PAYLOAD_TRACE_TCP_TX_SCHED_SENT, sock->id, tx_bump, ctx->core);
    tx_sock_tx_bump(sock, tx_bump);
    tx_sock_sched_next(sock, hdrlen + tx_bump, now_tsc);
    avail = sock_sched_avail(sock);

    /* Re-schedule if there are still bytes to send */
    if (avail > 0)
    {
      ret = sched_add(&ctx->sched, sock->id, tx_sched_tsc(sock), avail);
      if (ret < 0)
        return -1;
      tcp_payload_trace_add_to_msgs(payload, tx_bump,
          TCP_PAYLOAD_TRACE_TCP_SCHED_RESCHEDULED, sock->id, avail, ctx->core);
    }
  }
  
  util_spin_unlock(&sock->lock);
  return hdrlen + tx_bump;
}

int tcp_event_deq(struct cham_ebpf_ctx *ctx)
{
  __u8 qe_valid;
  
  qe_valid = is_qe_valid(ctx->qe, ctx->shm_end);
  if (!qe_valid)
    return -1;

  switch (ctx->qe->type)
  {
    case TCP_QUEUE_BUMP_CHAM_TX:
      return deq_handle_bump_tx(ctx);
    case TCP_QUEUE_BUMP_CHAM_RX:
      return deq_handle_bump_rx(ctx);
    case TCP_QUEUE_CTL_TX:
      return deq_handle_ctl_tx(ctx);
    case TCP_QUEUE_TX_RETRANSMIT:
      return deq_handle_retransmit(ctx);
    default:
      return -1;
  }
  return 0;
}

/***** Miscellaneous helpers *************************************************/

static inline __u8 is_qe_valid(struct queue_entry *qe, void *shm_end)
{
  void *qe_end;
  
  qe_end = (__u8 *) qe + sizeof(struct queue_entry);
  return qe_end < shm_end;
}

static inline __u8 is_qe_bump_valid(struct queue_entry *qe,
    void *shm_end)
{
  void *qe_end;

  qe_end = (__u8 *) qe + sizeof(struct tcp_queue_bump_entry);
  return qe_end < shm_end;
}

static inline __u8 is_qe_ctl_valid(struct queue_entry *qe,
    void *shm_end)
{
  void *qe_end;

  qe_end = (__u8 *) qe + sizeof(struct tcp_queue_ctl_entry);
  return qe_end < shm_end;
}

static inline __u8 is_qe_pkt_valid(struct queue_entry *qe,
    void *shm_end)
{
  void *qe_end;

  qe_end = (__u8 *) qe + sizeof(struct tcp_queue_pkt_entry);
  return qe_end < shm_end;
}

static inline __u8 is_pktlen_valid(void *pkt, void *pkt_end)
{
  return (pkt + sizeof(struct tcp_pkt_inner)) <= pkt_end;
}

static inline __u8 is_paylen_valid(void *pkt, 
    void *pkt_end, __u16 hdrs_len, __u16 paylen)
{
  return (pkt + hdrs_len + paylen) <= pkt_end;
}

static inline void remit(struct cham_scheduler *sched,
    struct tcp_sock *sock)
{
  __u32 avail;

  tcp_sock_recovery_enter(sock);
  
  /* Rewind socket */
  sock->tx_avail += sock->tx_pending;
  sock->tx_pending = 0;
  
  /* Remove any scheduled entries */
  sched_remove(sched, sock->id);
  
  /* Add a new retransmit entry */
  avail = sock_sched_avail(sock);
  sched_add(sched, sock->id, clock_rdtsc(), avail);
}

/***** RX helpers ************************************************************/

static inline __u32 rx_get_hdrlen(struct tcp_pkt_inner *tcp_pkt)
{
  __u16 ip_hdrs_len, tcp_hdrs_len;
  
  ip_hdrs_len = (__u16) (IPH_HL(&tcp_pkt->ip) * 4);
  tcp_hdrs_len = (__u16) (TCPH_HDRLEN(&tcp_pkt->tcp) * 4);
  return ip_hdrs_len + tcp_hdrs_len;
}

static inline __u32 rx_get_paylen(struct tcp_pkt_inner *tcp_pkt)
{
  __u16 ip_hdrlen, ip_totlen, tcp_hdrlen, paylen;
  
  ip_hdrlen = (__u16) (IPH_HL(&tcp_pkt->ip) * 4);
  ip_totlen = f_beui16(tcp_pkt->ip.len);
  tcp_hdrlen = (__u16) (TCPH_HDRLEN(&tcp_pkt->tcp) * 4);
  paylen = ip_totlen - ip_hdrlen - tcp_hdrlen;
  return paylen;
}

static inline __u8 rx_is_ip(struct tcp_pkt_inner *tcp_pkt)
{
  return IPH_V(&tcp_pkt->ip) == 4 && IPH_HL(&tcp_pkt->ip) >= 5 &&
      tcp_pkt->ip.proto == IP_PROTO_TCP;
}

static inline __u8 rx_is_ce(struct tcp_pkt_inner *tcp_pkt)
{
  return IPH_ECN(&tcp_pkt->ip) == CHAM_IP_ECN_CE;
}

static inline __u8 rx_is_flags_ctl(struct tcp_pkt_inner *tcp_pkt)
{
  __u16 flags;
  
  flags = TCPH_FLAGS(&tcp_pkt->tcp);
  return flags & (TAS_TCP_SYN | TAS_TCP_FIN | TAS_TCP_RST);
}

static inline __u8 rx_is_ack(struct tcp_pkt_inner *tcp_pkt)
{
  __u16 flags;
  flags = TCPH_FLAGS(&tcp_pkt->tcp);
  return flags & (TAS_TCP_ACK);
}

static inline __u8 rx_is_ack_only(struct tcp_pkt_inner *tcp_pkt)
{
  return rx_get_paylen(tcp_pkt) == 0;
}

static inline __u8 rx_is_payload_dup(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock)
{
  __u32 paylen, overlap;
  
  paylen = rx_get_paylen(tcp_pkt);
  overlap = rx_get_payload_overlap(tcp_pkt, sock);
  return paylen <= overlap;
}

static inline __u8 rx_is_out_of_order(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock)
{
  __u32 seqno;
  
  seqno = f_beui32(tcp_pkt->tcp.seqno);
  return (__s32) (seqno - sock->rx_seq) > 0;
}

static inline __u8 rx_is_ack_valid(__u32 seqno,
      __u32 pending, __u32 avail, __u32 ackno)
{
  __u32 max_ack;

  max_ack = seqno + pending + avail;
  if (seqno <= max_ack) {
    if (ackno < seqno || ackno > max_ack)
      return 0;
  } else {
    if (seqno > ackno && ackno > max_ack)
      return 0;
  }

  return 1;
}

static inline int rx_punt_ctl(struct cham_ebpf_ctx *ctx,
    struct tcp_pkt_inner *tcp_pkt)
{
  int ret;
  struct equeue *sig_q;
  struct equeue *pkt_q;
  struct tcp_ctl_cfg *cfg;
  struct tcp_queue_ctl_entry *sig_qe;
  struct tcp_queue_pkt_entry *pkt_qe;
  struct cham_map *map;
  __u32 hdrlen;
  __u16 core;

  /* Get control configuration containing queue ids */
  map = &ctx->maps[CFG_MAP];
  cfg = (struct tcp_ctl_cfg *) map->addr;
  if (cfg == NULL)
    return -1;
  core = ctx->core;
  if (core >= MAX_FP_CORES)
    return -1;

  /* Get first available entry in end of pkt queue */
  pkt_q = &ctx->equeues[cfg->fast_slow_pkt_qids[core]].eq;
  pkt_qe = queue_tail(pkt_q);
  if (pkt_qe == NULL)
    return -1;

  /* Get first available entry in end of signal queue */
  sig_q = &ctx->equeues[cfg->fast_slow_sig_qids[core]].eq;
  sig_qe = queue_tail(sig_q);
  if (sig_qe == NULL)
    return -1;

  hdrlen = rx_get_hdrlen(tcp_pkt);
  if (!is_paylen_valid(ctx->pkt, ctx->pkt_end, hdrlen, 0))
    return -1;

  /* Copy header to pkt queue */
  memcpy(&pkt_qe->data.ctl_pkt, tcp_pkt, hdrlen);
  pkt_qe->data.ctl_pkt.core = ctx->core;
  
  /* Set ready field of signal entry */
  sig_qe->data.ctl_sig.ready = 1;

  /* Push packet to queue */
  ret = queue_enqueue(pkt_q, TCP_QUEUE_CTL_RX_PKT);
  if (ret != 0)
  {
    return -1;
  }

  /* Push signal that packet is in queue */
  ret = queue_enqueue(sig_q, TCP_QUEUE_CTL_RX);
  if (ret != 0)
  {
    return -1;
  }

  return 0;
}

static inline __u32 rx_get_ack_bump(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock)
{
  __u8 is_valid;
  __u32 ackno, seqno, pending, avail;
  
  seqno = sock->tx_seq;
  ackno = f_beui32(tcp_pkt->tcp.ackno);
  pending = sock->tx_pending;
  avail = sock->tx_avail;

  is_valid = rx_is_ack_valid(seqno, pending, avail, ackno);
  if (is_valid)
  {
    return ackno - seqno;
  }
  else
  {
    return 0;
  }
}

static inline __u8 rx_read_ts(struct tcp_pkt_inner *tcp_pkt,
    void *pkt_end, __u32 *ts_val, __u32 *ts_ecr)
{
  __u32 hdrlen;
  struct tcp_timestamp_opt *ts_opt;
  void *opt_end;

  hdrlen = TCPH_HDRLEN(&tcp_pkt->tcp) * 4;
  if (hdrlen <= TCP_HLEN)
    return 0;

  ts_opt = (struct tcp_timestamp_opt *) ((__u8 *) &tcp_pkt->tcp + TCP_HLEN);
  opt_end = (__u8 *) ts_opt + sizeof(*ts_opt);
  if (opt_end > pkt_end)
    return 0;
  
  if (ts_opt->kind != TCP_OPT_TIMESTAMP)
    return 0;

  *ts_val = f_beui32(ts_opt->ts_val);
  *ts_ecr = f_beui32(ts_opt->ts_ecr);
  return 1;
}

static inline __u32 tcp_now_us(void)
{
  return (__u32) clock_tsc_to_us(clock_rdtsc());
}

static inline void sock_ts_rx(struct tcp_sock *sock,
    struct tcp_pkt_inner *tcp_pkt, void *pkt_end)
{
  __u32 now_us;
  __u32 rtt;
  __u32 ts_ecr;
  __u32 ts_val;

  if (!rx_read_ts(tcp_pkt, pkt_end, &ts_val, &ts_ecr))
    return;

  sock->ts_recent = ts_val;
  if (ts_ecr == 0)
    return;

  now_us = tcp_now_us();
  rtt = now_us - ts_ecr;
  if (rtt == 0 || rtt >= TCP_MAX_RTT)
    return;

  if (sock->rtt_est != 0)
    sock->rtt_est = (7 * sock->rtt_est + rtt) / 8;
  else
    sock->rtt_est = rtt;
}

static inline void rx_sched_tx(struct cham_ebpf_ctx *ctx,
    struct tcp_sock *sock, __u32 avail_before, __u32 avail_after)
{
  __u64 prio;

  if (avail_after <= avail_before)
    return;

  prio = tx_sched_tsc(sock);
  sched_add(&ctx->sched, sock->id, prio, avail_after - avail_before);
}

static inline __u32 rx_get_rx_bump(struct tcp_sock *sock, __u32 paylen)
{
  __u32 free_rx, rx_bump;
  
  free_rx = sock->rx_len - sock->rx_avail;
  rx_bump = paylen;
  if (free_rx < paylen)
  {
    rx_bump = free_rx;
  }
  
  return rx_bump;
}

static inline void rx_sock_bump_ack(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock, __u32 ack_bump)
{
  __u32 extra;

  extra = ack_bump > sock->tx_pending ? ack_bump - sock->tx_pending : 0;
  if (extra > sock->tx_avail)
    extra = sock->tx_avail;
  sock->tx_seq += ack_bump;
  if (ack_bump <= sock->tx_pending)
  {
    sock->tx_pending -= ack_bump;
  }
  else
  {
    sock->tx_pending = 0;
    sock->tx_avail -= extra;
  }
  sock->cc_ackb += ack_bump;
  if ((TCPH_FLAGS(&tcp_pkt->tcp) & TAS_TCP_ECE) != 0)
    sock->cc_ecnb += ack_bump;
  sock->tx_head = (sock->tx_head + ack_bump) % sock->tx_len;
  if (ack_bump > 0)
  {
    sock->rx_dupack_cnt = 0;
    tcp_sock_recovery_ack(sock);
  }
}

static inline void rx_sock_bump_rx(struct tcp_sock *sock, 
    __u32 rx_bump)
{
  sock->rx_seq = sock->rx_seq + rx_bump;
  sock->rx_avail += rx_bump;
}

static inline __u32 rx_get_payload_overlap(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock)
{
  __u32 seqno, overlap, paylen;
  
  seqno = f_beui32(tcp_pkt->tcp.seqno);
  overlap = 0;
  if ((__s32) (seqno - sock->rx_seq) < 0)
    overlap = sock->rx_seq - seqno;
    
  paylen = rx_get_paylen(tcp_pkt);
  if (overlap > paylen)
    overlap = paylen;

  return overlap;
}

static inline void rx_copy_payload(void *shm_base,
    struct tcp_sock *sock, void *payload, __u32 paylen)
{
  __u8 *rx_base;
  __u32 tail, part;
  
  rx_base = (__u8 *) shm_base + sock->rx_off;
  tail = sock->rx_head + sock->rx_avail;
  if (tail >= sock->rx_len)
    tail -= sock->rx_len;
  if (tail + paylen <= sock->rx_len)
  {
    memcpy(rx_base + tail, payload, paylen);
  }
  else
  {
    part = sock->rx_len - tail;
    memcpy(rx_base + tail, payload, part);
    memcpy(rx_base, (__u8 *) payload + part, paylen - part);
  }
}

static inline int rx_enqueue_rx_bump(struct equeue *q,
    struct tcp_sock *sock, __u32 rx_bump,
    __u16 rx_port, __u32 rx_ip)
{
  int ret;
  struct tcp_queue_bump_entry *qe;
  struct tcp_queue_bump_app_rx *bump;
  
  qe = queue_tail(q);
  if (qe == NULL)
  {
    return -1;
  }
  
  bump = &qe->data.bump_app_rx;
  bump->opaque = sock->opaque;
  bump->rx_avail = rx_bump;
  bump->rx_port = rx_port;
  bump->rx_ip = rx_ip;

  ret = queue_enqueue(q, TCP_QUEUE_BUMP_APP_RX);
  if (ret != 0)
    return -1;

  return 0;
}

static inline int rx_enqueue_tx_bump(struct equeue *q,
    struct tcp_sock *sock, __u32 tx_bump)
{
  int ret;
  struct tcp_queue_bump_entry *qe;
  struct tcp_queue_bump_app_tx *bump;
  
  qe = queue_tail(q);
  if (qe == NULL)
  {
    return -1;
  }
  
  bump = &qe->data.bump_app_tx;
  bump->opaque = sock->opaque;
  bump->tx_head = tx_bump;

  ret = queue_enqueue(q, TCP_QUEUE_BUMP_APP_TX);
  if (ret != 0)
    return -1;

  return 0;
}

/***** TX helpers ************************************************************/

static inline __u32 tx_get_hdrlen()
{
  return sizeof(struct ip_hdr) + TCP_HLEN + TCP_TS_OPT_LEN;
}

static inline int tx_fill_ack(struct cham_ebpf_ctx *ctx,
    struct tcp_sock *sock)
{
  __u8 pktlen_valid, paylen_valid;
  __u32 hdrlen;

  pktlen_valid = is_pktlen_valid(ctx->pkt, ctx->pkt_end);
  if (!pktlen_valid)
    return -1;

  hdrlen = tx_get_hdrlen();
  paylen_valid = is_paylen_valid(ctx->pkt, ctx->pkt_end, hdrlen, 0);
  if (!paylen_valid)
    return -1;

  tx_fill_hdr(ctx->pkt, sock, hdrlen, 0);
  return hdrlen;
}

static inline __u32 tx_get_max_paylen(struct cham_ebpf_ctx *ctx,
    __u32 hdrlen)
{
  __u32 payload_headroom;
  
  payload_headroom = (ctx->pkt_end - ctx->pkt);
  if (payload_headroom < hdrlen)
  {
    return 0;
  }
  
  return payload_headroom - hdrlen;
}

static inline void tx_write_tsopt(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock, __u32 now_us)
{
  struct tcp_timestamp_opt *ts_opt;

  ts_opt = (struct tcp_timestamp_opt *) ((__u8 *) &tcp_pkt->tcp + TCP_HLEN);
  ts_opt->kind = TCP_OPT_TIMESTAMP;
  ts_opt->length = sizeof(struct tcp_timestamp_opt);
  ts_opt->ts_val = t_beui32(now_us);
  ts_opt->ts_ecr = t_beui32(sock->ts_recent);
  ts_opt->pad = 0;
}

static inline void tx_write_tsopt_val(struct tcp_pkt_inner *tcp_pkt)
{
  __u32 now_us;
  struct tcp_timestamp_opt *ts_opt;

  ts_opt = (struct tcp_timestamp_opt *) ((__u8 *) &tcp_pkt->tcp + TCP_HLEN);
  now_us = tcp_now_us();  
  ts_opt->ts_val = t_beui32(now_us);
}

static inline __u32 tx_get_tx_bump(struct tcp_sock *sock,
    __u32 max_paylen)
{
  __u32 recovery_avail;
  __u32 tx_bump;
  __s64 fc_avail;
  
  tx_bump = sock->tx_avail;
  if (tx_bump > max_paylen)
    tx_bump = max_paylen;
    
  fc_avail = (__s64) sock->tx_remote_avail - (__s64) sock->tx_pending;
  if (fc_avail < 0)
    fc_avail = 0;
    
  if (tx_bump > fc_avail)
    tx_bump = fc_avail;

  recovery_avail = tcp_sock_recovery_avail(sock);
  if (tx_bump > recovery_avail)
    tx_bump = recovery_avail;
  
  return tx_bump;
}

static inline void tx_fill_hdr(struct tcp_pkt_inner *tcp_pkt,
    struct tcp_sock *sock, __u32 hdrlen, __u32 paylen)
{
  __u16 flags;
  __u32 now_us;
  __u32 rxwnd;
  /* Fill IP header */
  IPH_VHL_SET(&tcp_pkt->ip, 4, 5);
  tcp_pkt->ip._tos = 0;
  tcp_pkt->ip.len = t_beui16(hdrlen + paylen);
  tcp_pkt->ip.id = t_beui16(3);
  tcp_pkt->ip.offset = t_beui16(0);
  tcp_pkt->ip.ttl = 0xff;
  tcp_pkt->ip.proto = IP_PROTO_TCP;
  tcp_pkt->ip.chksum = 0;
  tcp_pkt->ip.src = t_beui32(sock->local_ip);
  tcp_pkt->ip.dst = t_beui32(sock->remote_ip);
  if ((sock->flags & TCP_SOCK_FLAG_ECN) && paylen > 0)
  {
    IPH_ECN_SET(&tcp_pkt->ip, CHAM_IP_ECN_ECT0);
  }
  
  /* Fill TCP header */
  flags = TAS_TCP_ACK;
  if (paylen > 0)
    flags |= TAS_TCP_PSH;
  if (sock->flags & TCP_SOCK_FLAG_SEND_ECE)
  {
    flags |= TAS_TCP_ECE;
    sock->flags &= ~TCP_SOCK_FLAG_SEND_ECE;
  }
  rxwnd = sock->rx_len - sock->rx_avail;
  tcp_pkt->tcp.src = t_beui16(sock->local_port);
  tcp_pkt->tcp.dest = t_beui16(sock->remote_port);
  tcp_pkt->tcp.seqno = t_beui32(sock->tx_seq + sock->tx_pending);
  tcp_pkt->tcp.ackno = t_beui32(sock->rx_seq);
  TCPH_HDRLEN_FLAGS_SET(&tcp_pkt->tcp, hdrlen / 4 - sizeof(struct ip_hdr) / 4,
      flags);
  tcp_pkt->tcp.wnd = t_beui16(rxwnd);
  tcp_pkt->tcp.chksum = 0;
  tcp_pkt->tcp.urgp = t_beui16(0);
  now_us = tcp_now_us();
  tx_write_tsopt(tcp_pkt, sock, now_us);
}

static inline void tx_copy_payload(void *shm_base, struct tcp_sock *sock,
    void *payload, __u32 tx_bump)
{
  __u32 tx_pos, part;
  
  tx_pos = sock->tx_head + sock->tx_pending;
  if (tx_pos >= sock->tx_len)
  {
    tx_pos -= sock->tx_len;
  }
  
  if (tx_pos + tx_bump <= sock->tx_len)
  {
    memcpy(payload, shm_base + sock->tx_off + tx_pos, tx_bump);
  }
  else
  {
    part = sock->tx_len - tx_pos;
    memcpy(payload, shm_base + sock->tx_off + tx_pos, part);
    memcpy(payload + part, shm_base + sock->tx_off, tx_bump - part);
  }
}

static inline void tx_sock_tx_bump(struct tcp_sock *sock,
    __u32 tx_bump)
{
  sock->tx_avail -= tx_bump;
  sock->tx_pending += tx_bump;
}

static inline __u64 tx_sched_tsc(struct tcp_sock *sock)
{
  __u64 now_tsc;

  now_tsc = clock_rdtsc();
  if (sock->cc_rate == 0 || sock->tx_ready_tsc <= now_tsc)
    return now_tsc;

  return sock->tx_ready_tsc;
}

static inline void tx_sock_sched_next(struct tcp_sock *sock,
    __u32 pktlen, __u64 now_tsc)
{
  __u64 delay_tsc;

  if (sock->cc_rate == 0)
  {
    sock->tx_ready_tsc = now_tsc;
    return;
  }

  delay_tsc = rate_delay_tsc(pktlen, sock->cc_rate);
  sock->tx_ready_tsc = now_tsc + delay_tsc;
}

/***** DEQ helpers ***********************************************************/

static inline int deq_handle_bump_tx(struct cham_ebpf_ctx *ctx)
{
  int ret;
  __u8 qe_valid;
  struct tcp_sock *sock;
  struct tcp_queue_bump_entry *qe;
  struct tcp_queue_bump_cham_tx *bump;
  struct cham_map *map;

  qe_valid = is_qe_bump_valid(ctx->qe, ctx->shm_end);
  if (!qe_valid)
    return -1;

  qe = (struct tcp_queue_bump_entry *) ctx->qe;
  bump = &qe->data.bump_cham_tx;

  /* Return if we can't find socket */
  map = &ctx->maps[SOCK_MAP];
  sock = sock_map_lookup(map, bump->sock_id);
  if (sock == NULL)
    return -1;

  /* Update socket state */
  util_spin_lock(&sock->lock);
  deq_sock_bump_tx(sock, bump);  
  util_spin_unlock(&sock->lock);
  
  /* Schedule bytes for transmission */
  ret = sched_add(&ctx->sched, sock->id, tx_sched_tsc(sock), bump->tx_avail);
  if (ret < 0)
    return -1;
  
  return 0;
}

static inline int deq_handle_bump_rx(struct cham_ebpf_ctx *ctx)
{
  __u8 qe_valid, rxfull;
  __u32 hdrlen;
  struct tcp_sock *sock;
  struct tcp_queue_bump_cham_rx *bump;
  struct tcp_queue_bump_entry *qe;
  struct cham_map *map;

  qe_valid = is_qe_bump_valid(ctx->qe, ctx->shm_end);
  if (!qe_valid)
    return -1;

  qe = (struct tcp_queue_bump_entry *) ctx->qe;
  bump = &qe->data.bump_cham_rx;

  /* Return if we can't find socket */
  map = &ctx->maps[SOCK_MAP];
  sock = sock_map_lookup(map, bump->sock_id);
  if (sock == NULL)
    return -1;
  
  util_spin_lock(&sock->lock);
  rxfull = sock->rx_len == sock->rx_avail;
  
  /* Update socket state */
  deq_sock_bump_rx(sock, bump);
  
  /* If the window opened, transmit a pure ACK immediately. */
  if (rxfull)
  {
    hdrlen = tx_fill_ack(ctx, sock);
    util_spin_unlock(&sock->lock);
    return hdrlen;
  }
  
  util_spin_unlock(&sock->lock);

  return 0;
}

static inline int deq_handle_ctl_tx(struct cham_ebpf_ctx *ctx)
{
  int ret;
  __u8 qe_valid, paylen_valid;
  __u32 hdrlen;
  struct cham_map *map;
  struct tcp_ctl_cfg *cfg;
  struct dqueue *ctl_pkt_q;
  struct tcp_queue_pkt_entry *ctl_pkt_qe;
  struct tcp_pkt_inner *ctl_pkt;
  
  qe_valid = is_qe_ctl_valid(ctx->qe, ctx->shm_end);
  if (!qe_valid)
    return -1;

  /* Return if we can't find map containing queue ids */
  map = &ctx->maps[CFG_MAP];
  cfg = (struct tcp_ctl_cfg *) map->addr;
  if (cfg == NULL)
    return -1;
  
  /* Get control packet */
  ctl_pkt_q = &ctx->dqueues[cfg->slow_fast_pkt_qid].dq;
  ctl_pkt_qe = queue_head(ctl_pkt_q);
  if (ctl_pkt_qe == NULL)
    return -1;
  qe_valid = is_qe_pkt_valid((struct queue_entry *) ctl_pkt_qe, ctx->shm_end);
  if (!qe_valid)
    return -1;
  ctl_pkt = &ctl_pkt_qe->data.ctl_pkt.pkt;
  
  /* Copy control packet for transmission */
  hdrlen = f_beui16(ctl_pkt->ip.len);
  paylen_valid = is_paylen_valid(ctx->pkt, ctx->pkt_end, hdrlen, 0);
  if (!paylen_valid)
    return -1;
  ctl_pkt->ip.chksum = 0;
  ctl_pkt->tcp.chksum = 0;
  memcpy(ctx->pkt, ctl_pkt, hdrlen);
  if (TCPH_HDRLEN(&ctl_pkt->tcp) * 4 == TCP_HLEN + TCP_TS_OPT_LEN)
  {
    if (!is_paylen_valid(ctx->pkt, ctx->pkt_end, tx_get_hdrlen(), 0))
      return -1;
    tx_write_tsopt_val(ctx->pkt);
  }
  
  ret = queue_dequeue(ctl_pkt_q);
  if (ret != 0)
    return -1;

  return hdrlen;
}

static inline int deq_handle_retransmit(struct cham_ebpf_ctx *ctx)
{
  __u8 qe_valid;
  struct tcp_sock *sock;
  struct tcp_queue_ctl_entry *qe;
  struct tcp_queue_ctl_remit *cmd;
  struct cham_map *map;
  
  qe_valid = is_qe_ctl_valid(ctx->qe, ctx->shm_end);
  if (!qe_valid)
    return -1;

  qe = (struct tcp_queue_ctl_entry *) ctx->qe;
  cmd = &qe->data.ctl_remit;

  /* Return if we can't find socket */
  map = &ctx->maps[SOCK_MAP];
  sock = sock_map_lookup(map, cmd->sock_id);
  if (sock == NULL)
    return -1;
  
  /* Rewind socket and schedule retransmission */
  util_spin_lock(&sock->lock);
  if (!deq_is_retransmit_stale(sock, cmd))
    remit(&ctx->sched, sock);
  util_spin_unlock(&sock->lock);
  
  return 0;
}

static inline __u8 deq_is_retransmit_stale(struct tcp_sock *sock,
    struct tcp_queue_ctl_remit *cmd)
{
  return sock->tx_pending == 0 || sock->tx_seq != cmd->tx_seq ||
      sock->tx_pending != cmd->tx_pending;
}

static inline __u32 deq_get_tx_ip(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_tx *bump)
{
  if (bump->tx_ip == 0)
  {
    return sock->remote_ip;
  }
  else
  {
    return bump->tx_ip;
  }
}

static inline __u32 deq_get_tx_port(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_tx *bump)
{
  if (bump->tx_port == 0)
  {
    return sock->remote_port;
  }
  else
  {
    return bump->tx_port;
  }
}

static inline __u32 deq_sock_bump_tx(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_tx *bump)
{
  __u32 tx_ip;
  __u32 tx_port;
  
  tx_ip = deq_get_tx_ip(sock, bump);
  tx_port = deq_get_tx_port(sock, bump);
  sock->tx_avail += bump->tx_avail;
  sock->remote_ip = tx_ip;
  sock->remote_port = tx_port;
  
  return 0;
}

static inline __u32 deq_sock_bump_rx(struct tcp_sock *sock,
    struct tcp_queue_bump_cham_rx *bump)
{
  __u32 new_head;
  
  new_head = sock->rx_head + bump->rx_head;
  if (new_head >= sock->rx_len)
    new_head -= sock->rx_len;
  
  sock->rx_head = new_head;
  sock->rx_avail -= bump->rx_head;
  
  return 0;
}

/***** Socket helpers ********************************************************/

static inline struct tcp_sock * sock_find(struct cham_ebpf_ctx *ctx,
    struct tcp_pkt_inner *tcp_pkt)
{
  int i;
  __u16 lport, rport;
  __u32 sid;
  __u32 hash;
  __u32 lip, rip;
  struct tcp_flow_bucket *bucket;
  struct tcp_sock *sock;
  struct cham_map *map;

  rip = f_beui32(tcp_pkt->ip.src);
  lip = f_beui32(tcp_pkt->ip.dst);
  rport = f_beui16(tcp_pkt->tcp.src);
  lport = f_beui16(tcp_pkt->tcp.dest);
  
  hash = sock_hash(lip, lport, rip, rport) % TCP_FLOW_BUCKETS;

  /* Get map hash buckets with potential socket ids */
  map = &ctx->maps[SID_BUCK_MAP];
  if (hash >= map->nelems)
    return NULL;
  bucket = (struct tcp_flow_bucket *) ((__u8 *) map->addr +
      (hash * map->elsize));
  if (bucket == NULL)
    return NULL;

  /* Iterate over hash buckets to get actual id */
  map = &ctx->maps[SOCK_MAP];
  for (i = 0; i < TCP_FLOW_BUCKET_SLOTS; i++)
  {
    sid = bucket->sids[i];
    if (sid == ID_INVALID)
      continue;

    sock = sock_map_lookup(map, sid);
    if (sock == NULL)
      continue;

    if (sock->state == TCP_SOCK_STATE_CLOSED)
      continue;

    if (sock->local_ip == lip && sock->local_port == lport &&
        sock->remote_ip == rip && sock->remote_port == rport)
      return sock;
  }

  return NULL;
}

static inline struct tcp_sock *sock_map_lookup(struct cham_map *map,
    __u32 sid)
{
  if (sid >= map->nelems)
    return NULL;

  return (struct tcp_sock *) ((__u8 *) map->addr + (sid * map->elsize));
}

static inline __u32 sock_hash(__u32 lip, __u16 lport,
    __u32 rip, __u16 rport)
{
  __u32 h;

  h = lip ^ rip ^ ((__u32) lport << 16) ^ rport;
  return h ^ (h >> 16);
}

static inline __u32 sock_sched_avail(struct tcp_sock *sock)
{
  __u32 fc_avail;
  __u32 recovery_avail;
  __u32 avail;
  
  if (sock->tx_pending >= sock->tx_remote_avail)
    return 0;
    
  fc_avail = sock->tx_remote_avail - sock->tx_pending;
  avail = fc_avail;
  if (sock->tx_avail < fc_avail)
    avail = sock->tx_avail;
  recovery_avail = tcp_sock_recovery_avail(sock);
  if (avail > recovery_avail)
    avail = recovery_avail;
  return avail;
}
