...
static int state_read(const char *path, struct rpc_state *state);
...
static void apps_print(pid_t pid, const struct rpc_slow_context *ctx,
    __u32 n_fp_cores);
...
int main(int argc, char **argv)
{
  ...
  path = argc > 1 ? argv[1] : RPC_STATE_PATH;
  if (state_read(path, &state) != 0)
    return 1;

  if (state.magic != RPC_STATE_MAGIC || state.version != RPC_STATE_VERSION)
  {
    fprintf(stderr, "bad RPC state in %s\n", path);
    return 1;
  }
  if (state.ctx_size != sizeof(ctx) ||
      // state.sock_size != sizeof(struct udp_sock) ||
      // state.port_size != sizeof(struct udp_port) ||
      // state.cfg_size != sizeof(struct rpc_cfg) ||
      state.app_size != sizeof(struct rpc_app_slow) ||
      state.app_ctx_size != sizeof(struct rpc_app_context_slow))
  {
    fprintf(stderr, "RPC state ABI mismatch, rebuild rpc_slow and statetool\n");
    return 1;
  }

  pid = state.pid;
  if (proc_read(pid, state.ctx_addr, &ctx, sizeof(ctx), "rpc_slow_context") != 0)
    return 1;
  ...
  {
    fprintf(stderr, "unexpected RPC table dimensions\n");
    return 1;
  }

  printf("rpc_slow pid=%u ctx=0x%" PRIx64 " n_socks=%u n_apps=%u\n",
...
static void apps_print(pid_t pid, const struct rpc_slow_context *ctx,
    __u32 n_fp_cores)
{
  const struct rpc_app_slow *app;
  const struct rpc_app_context_slow *actx;
  __u32 app_id;
  __u32 ctx_id;
  __u32 core;
