...
static int uxsocket_init(struct rpc_slow_context *ctx);
static int uxsocket_init_fd(struct rpc_slow_context *ctx);
static int uxsocket_accept(struct rpc_slow_context *ctx);
static void uxsocket_error(struct rpc_slow_context *ctx, struct app_event *aev);
static void uxsocket_receive(struct rpc_slow_context *ctx, struct app_event *aev);
...
static void uxsocket_receive(struct rpc_slow_context *ctx, struct app_event *aev)
{
  ...
  q = cham_new_queue(ctx->proto, APPQ_SZ, sizeof(struct rpc_queue_entry));
  if (q == NULL)
  {
  ...
  for (i = 0; i < ctx->proto->n_fp_cores; i++)
  {
    ...
    q = cham_new_queue(ctx->proto, BUMPQ_SZ, sizeof(struct rpc_queue_bump_entry));
    if (q == NULL)
    {
  ...
  /* Send out response */
  res_sz = sizeof(struct rpc_queue_new_actx_res);
