...
static int config_defaults(struct rpc_configuration *c);
static int parse_u32(const char *s, __u32 *val);
static int parse_block(const char *s, char *ebpf_path, size_t ebpf_path_len);
static void print_usage(struct rpc_configuration *c, char *progname);

int rpc_config_parse(struct rpc_configuration *c, int argc, char **argv)
{
  int ret, done = 0;
...
static int config_defaults(struct rpc_configuration *c)
{
  ...
  c->bumpq_len = 65536;
  c->ctlq_len = 1024;
  strcpy(c->ebpf_path, RPC_EBPF_BYTECODE);
  return 0;
}
...
static int parse_block(const char *s, char *ebpf_path, size_t ebpf_path_len)
{
  ...
  if (strcmp(s, "64") == 0)
    path = RPC_EBPF_BYTECODE_64;
  else if (strcmp(s, "128") == 0)
    path = RPC_EBPF_BYTECODE_128;
  else if (strcmp(s, "256") == 0)
    path = RPC_EBPF_BYTECODE_256;
  else if (strcmp(s, "512") == 0)
    path = RPC_EBPF_BYTECODE_512;
  else if (strcmp(s, "1024") == 0)
    path = RPC_EBPF_BYTECODE_1024;
  else
  {
...
static void print_usage(struct rpc_configuration *c, char *progname)
{
  fprintf(stderr, "Usage: %s [OPTION]...\n"
      ...
      "  --ctlq-len=NELEMS                      Control queue length in elements"
          " [default: %u]\n"
      "  --block=NR                             Select block-sized RPC fast path"
          " (64, 128, 256, 512, 1024)"
          " [default: none]\n"
      "\n"
      "Virtualization:\n"
